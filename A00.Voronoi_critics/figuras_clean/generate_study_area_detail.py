...
# -*- coding: utf-8 -*-
"""
Generate Study Area Detail Figure (Figure 6b)
==============================================

Two-panel figure (wider than the original 3-panel version):
  (A) Municipalities and CDW plants — scatter + Voronoi edges + boundary
  (B) Topographic stratification — zones by latitude + boundary

Uses trilateration for 46 plant positions.
"""
...
ZONE_PLAIN_PIED = 4_323_000
ZONE_PIED_MOUNT = 4_400_000
...
def generate_figure():
    ...
    # ============================================================ #
    # Panel (A): Municipalities and CDW plants
    # ============================================================ #
    # Voronoi edges
    for seg in segments:
        ax_a.plot(seg[:, 0] / 1000, seg[:, 1] / 1000,
                  color='#BBBBBB', lw=0.5, alpha=0.5, zorder=1)

    ax_a.scatter(muni_x / 1000, muni_y / 1000, c='#5B9BD5', s=20,
                 alpha=0.6, zorder=3, label=f'Municipalities ({n_munis})')
    ax_a.scatter(plants[:, 0] / 1000, plants[:, 1] / 1000,
                 c='#2E7D32', s=100, marker='^', zorder=5,
                 edgecolors='black', linewidth=0.5,
                 label=f'CDW Plants ({n_plants})')

    cookie_cutter(ax_a, bnd_x / 1000, bnd_y / 1000,
                  xmin / 1000, xmax / 1000, ymin / 1000, ymax / 1000)

    ax_a.set_aspect('equal')
    ax_a.set_xlim(bx_min / 1000 - 5, bx_max / 1000 + 5)
    ax_a.set_ylim(by_min / 1000 - 5, by_max / 1000 + 5)
    ax_a.set_xlabel('UTM Easting (km)', fontsize=12)
    ax_a.set_ylabel('UTM Northing (km)', fontsize=12)
    ax_a.set_title('(A) Municipalities and CDW plants', fontweight='bold')
    ax_a.legend(loc='lower right', fontsize=10, framealpha=0.9)

    # Scale bar
    sb_x, sb_y = bx_min / 1000 + 10, by_min / 1000 + 5
    ax_a.plot([sb_x, sb_x + 50], [sb_y, sb_y], 'k-', lw=3, zorder=9)
    ax_a.text(sb_x + 25, sb_y + 4, '50 km', ha='center', fontsize=9, zorder=9)

    # ============================================================ #
    # Panel (B): Topographic stratification
    # ============================================================ #
    zone_colors = {
        'Mountain': '#8B6914',
        'Piedmont': '#E5A520',
    ...
    for zone, color in zone_colors.items():
        ...
        ax_b.scatter(muni_x[mask] / 1000, muni_y[mask] / 1000,
                     c=color, s=30, alpha=0.7, zorder=3,
                     label=f'{zone} ({count}, {pct:.0f}%)')

    # Zone dividing lines
    ax_b.axhline(ZONE_PLAIN_PIED / 1000, color='#888888', lw=0.8,
                 ls=':', zorder=2)
    ax_b.axhline(ZONE_PIED_MOUNT / 1000, color='#888888', lw=0.8,
                 ls=':', zorder=2)

    # Zone labels on right side
    right_x = bx_max / 1000 + 2
    ax_b.text(right_x, (by_max / 1000 + ZONE_PIED_MOUNT / 1000) / 2,
              'Sistema\nCentral', ha='left', va='center', fontsize=9,
              color='#8B6914', fontstyle='italic')
    ax_b.text(right_x, (ZONE_PIED_MOUNT / 1000 + ZONE_PLAIN_PIED / 1000) / 2,
              'Piedmont\nzone', ha='left', va='center', fontsize=9,
              color='#E5A520', fontstyle='italic')
    ax_b.text(right_x, (ZONE_PLAIN_PIED / 1000 + by_min / 1000) / 2,
              'Tagus-Guadiana\nbasins', ha='left', va='center', fontsize=9,
              color='#7CB342', fontstyle='italic')

    # s-hat annotations on left side
    left_x = bx_min / 1000 + 5
    ax_b.text(left_x, (by_max / 1000 + ZONE_PIED_MOUNT / 1000) / 2,
              f'1,000\u20132,400 m\n$\\hat{{s}} = {S_MOUNTAINS}$',
              ha='left', va='center', fontsize=9, color='#8B6914')
    ax_b.text(left_x, (ZONE_PIED_MOUNT / 1000 + ZONE_PLAIN_PIED / 1000) / 2,
              f'200\u2013600 m\n$\\hat{{s}} = {S_PIEDMONT}$',
              ha='left', va='center', fontsize=9, color='#E5A520')
    ax_b.text(left_x, (ZONE_PLAIN_PIED / 1000 + by_min / 1000) / 2,
              f'<300 m\n$\\hat{{s}} = {S_PLAINS}$',
              ha='left', va='center', fontsize=9, color='#7CB342')

    # Boundary
    ax_b.plot(bnd_x / 1000, bnd_y / 1000, color='#333333', lw=1.2, zorder=6)

    ax_b.set_aspect('equal')
    ax_b.set_xlim(bx_min / 1000 - 5, bx_max / 1000 + 15)
    ax_b.set_ylim(by_min / 1000 - 5, by_max / 1000 + 5)
    ax_b.set_xlabel('UTM Easting (km)', fontsize=12)
    ax_b.set_ylabel('UTM Northing (km)', fontsize=12)
    ax_b.set_title('(B) Topographic stratification', fontweight='bold')
    ax_b.legend(loc='lower right', fontsize=9, framealpha=0.9,
                title='Topographic zones', title_fontsize=10)
    ...
    for d in [script_dir, fig_dir]:
        if os.path.isdir(d):
            p = os.path.join(d, 'study_area_detail.pdf')
            fig.savefig(p)
            print(f'  Saved: {p}')
    plt.close()
...
if __name__ == '__main__':
    print('=' * 60)
    print('GENERATING STUDY AREA DETAIL FIGURE')
    print('=' * 60)
    generate_figure()
    print('\nDone.')
