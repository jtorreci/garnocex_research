...
def generate_figure():
    ...
    # Largest-remainder (Hamilton) rounding so legend percentages sum to 100
    zone_counts = {z: int((zones == z).sum()) for z in zone_colors}
    raw_pcts = {z: 100 * c / n_munis for z, c in zone_counts.items()}
    floors = {z: int(p) for z, p in raw_pcts.items()}
    remainder = 100 - sum(floors.values())
    order = sorted(raw_pcts, key=lambda z: raw_pcts[z] - floors[z], reverse=True)
    pct_int = dict(floors)
    ...
    for zone, color in zone_colors.items():
        ...
        ax_c.scatter(muni_x[mask] / 1000, muni_y[mask] / 1000,
                     ...
                     label=f'{zone} ({count}, {pct_int[zone]}%)')
