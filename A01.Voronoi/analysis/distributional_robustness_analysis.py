...
def load_beta_data():
    """Load or generate beta scaling factors data"""
    # Check if real data exists
    data_file = "detailed_ratios_analysis.csv"
    try:
        df = pd.read_csv(data_file)
        beta_values = df['Ratio'].values
        print(f"Loaded {len(beta_values)} real beta values from {data_file}")
        return beta_values
    except FileNotFoundError:
        print("Real data not found. Generating synthetic data based on paper parameters...")
        # Generate synthetic data matching paper parameters
        # μ = 0.166, σ = 0.093 (log-normal parameters from paper)
        n_samples = 9240  # From paper: 9,240 municipality-plant pairs
        mu_log = 0.166
        sigma_log = 0.093
        beta_synthetic = np.random.lognormal(mu_log, sigma_log, n_samples)

        # Add some realistic noise and constraints
        beta_synthetic = np.clip(beta_synthetic, 0.8, 5.0)  # Realistic bounds
        print(f"Generated {len(beta_synthetic)} synthetic beta values")
        return beta_synthetic
